...
const CACHE_NAME='vocab-trainer-v12.8.9';
const APP_SHELL=['./index.html?v=12.8.9','./manifest.webmanifest','./icon-192.png','./icon-512.png'];
...
self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;
  ...
  try{const r=await fetch(e.request);if(r.ok&&new URL(e.request.url).origin===location.origin){c.put(e.request,r.clone());}return r;}catch(err){
    if(e.request.mode==='navigate'){const fb=await c.match('./index.html?v=12.8.9');if(fb)return fb;} throw err;}})());
});
